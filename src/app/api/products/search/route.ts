...
export async function GET(req: Request) {
  ...
  try {
    const tsRes = await typesenseServer
    ...
    // 3. Extract SKUs
    const hitSkus = uniqKeepOrder(
      (tsRes.hits || [])
        .map((h: any) => String(h.document.sku || h.document.id || "").trim())
        .filter(Boolean)
    );
    ...
    return NextResponse.json(
      {
        ...
        items,
        total_count: found,
        page_info: { current_page: page, total_pages: totalPages },
      },
